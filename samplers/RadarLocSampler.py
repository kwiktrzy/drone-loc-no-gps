# Source: https://github.com/jac99/RadarLoc/blob/main/datasets/samplers.py
...
class RadarLocSampler(Sampler):
   # Sampler returning list of indices to form a mini-batch
    ...
    # Image per place - positive examples per group
    def __init__(self, dataframe: pd.DataFrame, image_per_place: int, batch_size: int, batch_size_limit: int = None,
        ...
        if image_per_place > 2:
                assert image_per_place > 2., 'Image per place must be at least 2'
            
        self.k = image_per_place  # Number of positive examples per group must be 2
