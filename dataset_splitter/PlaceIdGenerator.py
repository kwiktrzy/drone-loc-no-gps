...
class PlaceIdGenerator:
    def __init__(
        self,
        csv_tiles_paths: List[str] = [],
        place_id_round=4,
        is_validation_set=False,
        force_regenerate=False,
    ):
        self.csv_tiles_paths = csv_tiles_paths
    ...
    def __calculate_place_id_for_all(self):
        ...
        for csv_path in self.csv_tiles_paths:
            df = pd.read_csv(csv_path)

            if "place_id" in df.columns and not self.force_regenerate:
                print(
                    f" Skipping Place ID generation for {csv_path}, 'place_id' column already exists."
                )
                continue
            ...
            df.loc[sat_mask, "place_id"] = df.index.to_series()[sat_mask].astype(
                "Int64"
            )

            for col in [
                "uav_to_nearest_sat_m",
                "uav_to_second_sat_m",
                "nearest_sats_pair_distance_m",
            ]:
                if col not in df.columns:
                    df[col] = np.nan

            good_coords = (
                df["e_utm"].notna() & df["n_utm"].notna() & df["zone_utm"].notna()
            )
            ...
            for zone, idxs in sub.groupby("zone_utm").groups.items():
                ...
                nearest_rel_sorted = nearest_rel[
                    rows, np.argsort(dist2[rows, nearest_rel], axis=1)
                ]

                sat_idx_arr = sat_idx.to_numpy()
                nearest_sat_global = sat_idx_arr[nearest_rel_sorted[:, 0]]

                df.loc[uav_idx, "place_id"] = df.loc[
                    nearest_sat_global, "place_id"
                ].to_numpy()
